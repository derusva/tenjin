...
> **产品状态更新（2026-09-04）**：Repo 保留，产品停止。日常记录与复习已迁移到 ChatGPT + Notion，主动产品开发暂停。下文保留原设计及当时的评审状态，仅作历史参考；当前项目状态见 [README.md](README.md)。
...
| 手工输入短词 | 不含打字时间 ≤5 秒 |
| Photos 单张截图 | 不含系统选图时间 ≤8 秒 |
| 表达纠正句对 | ≤15 秒，不纳入高频红线 |

采集最多作一个分类选择，不要求填写读音、原因、难度和标签。图片入口只接受经内容签名识别、且当前浏览器能实际预览的单张 JPEG、PNG、HEIC 或 HEIF，单张上限 20 MB；首版不做多图、OCR、语音转写或 GPT 解析。

图片作为可降级 context 保存在当前主屏 PWA 的 IndexedDB 中，记录 Blob、媒体类型、文件名、字节数和 SHA-256 摘要。图片摘要参与 context hash；Blob 不进入核心事件 payload，不上传远端。图片 context 缺失时，事件账本和 R/L/P 派生仍可正常重建。
...
- 只有非空答案才构成 R 通道可揭示复习材料；图片与答案在复习时一起显示；
- 没有答案的纯截图只保留 capture、预览与最近记录，不创建以文件名冒充内容的学习 item；带文字的无答案查词仍可搜索，但两者都不强塞进复习；
...
Shortcut 不是当前产品路径，也不要求用户手工搭建任何 action。文字和单图均从主屏 Tenjin 内完成；候选流程只保留为未来试验：分享所选文本 → Shortcut 封装临时 payload → 尝试打开 capture URL。
...
| 通道 | 无 LLM 可复习条件 |
|---|---|
| R | item + 非空的用户答案；有本机图片时与答案一起作为提示材料 |
...
- 主输入：粘贴 / 输入文字，或从系统选择单张图片；
- 三个通道入口：查过、没听出、表达纠正；
- 查过允许选填答案；无答案记录不进入 R 复习；
- 次按钮：复习 5 条、搜索；
- 不显示连续天数、欠账、排行榜或复杂统计。

### 11.2 采集页

- 单条输入为主；允许文字、单张本机图片或两者并存；表达纠正允许句对；
- 图片选择后先显示预览，可更换或移除；
- 最多一个分类选择；
- 保存后 8 秒撤销；
- 文字和图片均为空时不保存；
...
| v0.6 | 学习画像与单图切片 | GPT 学习总结拆为私有画像、公开解释策略和待验证假设；主 PWA 增加应用内单图记录、可选查词答案与 R 复习材料，不引入 OCR / GPT / Shortcut 依赖 | Codex | 2026-07-30 |

---

## 19. 当前状态结论

**当前可用：** 主 PWA 已实现本机记录、最近记录、撤销、复习、搜索和数据状态；日常文字从首页粘贴或输入，Photos 截图从首页选择单张图片。图片与可选查词答案保存在本机 context；填写答案的查词项可显示图片并进入 R 复习。两条路径都不要求 Shortcut。

**尚未交付：** Capture Inbox、多图、OCR、语音转写、GPT 解析、跨设备图片同步与图片完整备份。`capture-spike.html` 仍是只读、内存态的 Stage A 开发者诊断，不导入账本，也不能作为 Inbox、Shortcut 安装或跨应用稳定捕获的交付证据。

Shortcut 只有在能向用户交付一键安装的已签名成品，并通过真实 iPhone 的安装、分享、重启、失败反馈与稳定性 Gate 后，才可重新进入普通产品界面。不得要求用户手工搭建动作。

本轮可复验的自动化、浏览器证据和真实 iPhone 操作清单见 [`docs/qa/2026-07-30-learning-image-capture.md`](docs/qa/2026-07-30-learning-image-capture.md)。

...
外加**三条 must-not-fail** 用例，必须仍然 PASS——防止验证器过敏到只能靠字节巧合通过：

1. `events.jsonl` **行序打乱**；
2. events 各行内 JSON **对象键序打乱**；
3. **真实 undo 正例**：用真实采集路径造一条 capture，再用真实 `appendDiscard` 撤销（它会 GC 掉 context），然后 export → restore → 等价验证全绿。守的是 §5.3 的 active-capture 规则：一个"已撤销 capture 且 context 已被 GC"的**合法**账本，绝不能被误判为损坏。

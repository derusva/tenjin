...
> **⚠️ 2026-08-05 状态变更：近期实施范围已作废（superseded）。**
>
> 用户于 2026-08-05 明确授权作废本 PRD 的近期实施范围。原因见 [`docs/decisions/2026-08-05-external-collection-direction.md`](../decisions/2026-08-05-external-collection-direction.md)：本 PRD 的核心前提是「先收下、稍后理解」（捕获与语义处理分离，靠 iOS 分享菜单批量投递、事后集中处理），而用户的真实行为与之相反——阅读中遇到卡点即刻截图问外部 Coach，当场获得理解。因此本 PRD 的阶段门槛（阶段 B 的 10 次真实捕获与 5 条延迟处理、阶段 C 的 50 次捕获与 20 条延迟处理、第 16 章的完成定义）无法从用户实际使用中取得数据。
>
> 下面按条款列出去留，**不按整章继承**。

> **① 继承为规范性约束（已按新方向改写，措辞以决策记录为准）**
>
> - **10.4 REC-03 的原则**：恢复前完成全部校验，失败不得留下半恢复状态。原则原样继承。
> - **10.4 REC-02 的意图**（非其字段清单）：完整状态备份必须覆盖影响幂等与恢复正确性的全部状态。REC-02 原文列举的 Inbox 元数据、`sourceShardMonth`、`sourcePackageRelativePath`、`localDigestVersion`、`localPackageDigest` 绑定 Inbox 形态，**不继承**；改写后的原则见决策记录 §2.5。
> - **12.1 末条**：用户执行隐私清理时，append-only 不得高于用户删除权。

> **② 保留为参考，不构成当前需求**
>
> - **第 8 章 Capture Package 候选 v1 最小契约**（8.1 目录 / 8.2 Manifest 最小字段 / 8.3 本地摘要 / 8.4 MVP 限制）：仅在未来重新启用文件投递时适用。**它不是 Coach 路的输入契约**，不要照它设计 `tenjin.coach-transfer`。
> - **第 15 章 主要风险与缓解**：风险清单，供后续设计对照，不是需求。
> - **第 12.2 节 未来 GPT 的前置条件**：该节原文即自标 Informative / non-normative；其隐私意图（分析副本分离、去 EXIF/GPS、URL 清洗、不发送全量画像）在 Coach 路上无法由代码执行，只能作为纪律，见决策记录 §2.6。

> **③ 随近期范围一并作废**
>
> 第 6 章分阶段交付、第 7 章用户流程、**第 9 章 Inbox 数据模型**、10.1–10.3 功能需求、第 11 章指标与决策门（含 11.3 的 GPT 准入门槛）、第 13 章明确后置、第 14 章验收测试、**第 16 章 MVP 完成定义（含 50 次捕获完成条件）**、第 17 章复审结论与冻结规则。
>
> 关于第 17 章：其中「范围扩大必须先修改 PRD 版本并重新批准」这一条约束的是**本 PRD 阶段 A 的实施**，近期范围作废后它不再管辖新方向。它体现的纪律（范围变化要有书面、独立的批准）由上述决策记录承接，不因本章作废而失效。

> **④ 恢复条件（两条同时成立才可重新提请批准）**
>
> 1. 真实观察到「必须先收下、稍后再理解」的场景并有记录——即在无法即时求解释的环境下（无网、不便中断阅读、不便掏手机问 Coach）确实积累了延迟处理的需求；
> 2. 该需求无法用更简单的手段解决（例如直接用系统备忘录暂存、事后再问 Coach）。
>
> **Coach 路的 Stage 0 判定不通过不构成恢复条件。** 格式不稳或触发意愿不足只说明 Coach 路不成立，并不说明「先收后理」的需求出现了。作废不等于永久否定，但重新启用必须有它自己的证据。

**状态：** 近期实施范围已作废（2026-08-05）；原状态为「已批准并冻结；仅阶段 A GO」

**日期：** 2026-07-12（作废批注 2026-08-05）

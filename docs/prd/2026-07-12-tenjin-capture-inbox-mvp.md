...
| 最低系统 | iOS 18.4；目录导入依赖该版本的 `webkitdirectory` |
...
- 支持的来源都能稳定取得至少一种有用表示；
- 固定目录在首次授权后不再成为日常询问；
- 快捷指令写出的包可以由 iOS 18.4 主屏 PWA 重新选择目录并读回；
...
- 单图和一次 2–10 张多图；
- 按 UTC 月份保存的 CaptureLog；
- payload 先写、`capture.json` 最后写；
- Tenjin 中最小月份目录导入；
- 有序 `payloads[]` 的 Inbox；
- 用户可人工晋升为 R / L / P 学习项，或归档 / 删除；
...
- 路径只能指向当前 capture 内部，拒绝绝对路径、`..`、反斜杠和 NUL；
- `payloadId` 与 `path` 在包内唯一；
- v1 `kind` 只允许 `text`、`url` 和 `image`；
- v1 支持 `selection`、`page-title`、`page-url`、`attachment` 和 `unknown` role；
- `originalName` 是允许的可选显示字段；缺失时在本地摘要输入中使用 `null`；
- `byteLength` 和 `sha256` 是允许的可选完整性字段；是否由正式快捷指令写入以阶段 A 真机性能为准；
...
2. 按 manifest 顺序构造固定键序、无多余空白的 UTF-8 JSON，包含 `localDigestVersion`、`schemaVersion`、`captureId`、`capturedAt`、`transport`，以及每项的 `payloadId`、`kind`、`role`、`path`、`mediaType`、`originalName` 或 `null`、实际 `byteLength` 和实际 SHA-256；
...
- delete 删除本机可见内容，但 ImportLedger 保留最小 `captureId` 墓碑，避免下次导入复活；
- 用户人工整理或未来接受 proposal 时，才创建正式学习事件；
...
| 晋升为一个或多个学习项 | reviewed | archived | available | 从待处理 Inbox 移出，仍可搜索来源 |
| 明确归档但不晋升 | reviewed | archived | available | 表示已判断但本次不形成学习项 |
| 删除 capture | reviewed | deleted | removed | 删除本机 payload，仅保留 captureId、摘要和删除墓碑 |
...
指标口径：`processed` 指发生晋升、明确归档或删除，等价于 `reviewState = reviewed`；7 天处理率的分母只包含捕获时间已满 7 天的 capture；转化率为“至少产生一个正式 item 的 capture / reviewed capture”。
...
| ID | 优先级 | 需求 |
|---|---|---|
| CAP-01 | Must | “存入 Tenjin”可从 iOS 分享菜单运行 |
| CAP-02 | Must | 正常捕获不显示分类、备注或确认表单 |
| CAP-03 | Must | 每次调用只生成一个 captureId，多图保持同组和顺序 |
| CAP-04 | Must | payload 先写、manifest 最后写；只有全部必需写入成功才显示“已收下” |
| CAP-05 | Must | 首次选定目录后，日常捕获不再询问保存位置 |
| CAP-06 | Must | 空输入、权限、空间、转换和保存失败时不误报成功 |
| CAP-07 | Must | 通过共享链接交付，用户不手工搭建内部动作 |
| CAP-08 | Must | v1 类型归一化规则必须来自阶段 A 的真机样本，而不是只靠 Content Graph 文档推断 |

### 10.2 导入

| ID | 优先级 | 需求 |
|---|---|---|
| IMP-01 | Must | iOS 18.4 主屏模式允许用户选择单个月份目录 |
| IMP-02 | Must | 在 IndexedDB 事务外读取、限制大小并计算本地摘要 |
| IMP-03 | Must | 用 captureId + localPackageDigest 区分新增、重复和 ID 冲突；不同 captureId 的相同内容仍是独立 encounter |
| IMP-04 | Must | 单包元数据、payload Blob 和 ImportLedger 在短事务中原子写入 |
| IMP-05 | Must | 正常导入时，manifest 或引用文件暂不可用只进入本次“暂不可用”报告，不创建部分 InboxCapture；以后重选重试 |
| IMP-06 | Must | 不实现 24 小时 / 三次扫描状态机；MVP 不把等待同步误包装成永久损坏 |
| IMP-07 | Must | ImportLedger 保留已导入 ID 和删除墓碑 |
| IMP-08 | Must | 图片按允许的媒体类型和可识别文件签名校验，不只信任扩展名；不支持 SVG 或其他可执行内容 |
| IMP-09 | Must | 导入前在浏览器支持时读取存储估算，并始终统计 Tenjin 已存 payload 字节；达到 200 MB 软上限或发生 quota 错误时停止该包、保留 iCloud 原件并允许清理后重试 |

### 10.3 Inbox 与人工晋升

| ID | 优先级 | 需求 |
|---|---|---|
| INB-01 | Must | InboxCapture 不要求 R / L / P 分类 |
| INB-02 | Must | 按原顺序显示文字、URL、图片、role 和捕获时间 |
| INB-03 | Must | 支持按时间浏览、搜索文字 / URL、查看附件 |
| INB-04 | Must | 支持 reviewed、archived 和 deleted 的显式操作 |
| INB-05 | Must | 用户可把 capture 人工转成查过、没听出或表达纠正 |
| INB-06 | Must | 正式事件和 item 保留 captureId / payloadId 来源引用 |
| INB-07 | Should | 批处理界面一次最多展示 5 条资料，不用红点、欠账或连续天数催促清空 |

### 10.4 最小状态恢复

| ID | 优先级 | 需求 |
|---|---|---|
| REC-01 | Must | 进入持续学习试用前可导出版本化状态 JSON |
| REC-02 | Must | 状态 JSON 包含正式事件、contexts、Inbox 元数据、ImportLedger / 墓碑和事件坐标高水位 |
| REC-03 | Must | MVP 只支持校验后恢复到空库；失败不能留下半恢复状态 |
| REC-04 | Must | 状态 JSON 不重复打包 CaptureLog payload；恢复后将其标为 `needs-restore`，重新选择相关月份目录并核对 captureId / localPackageDigest 后补回 Blob |
| REC-05 | Must | 界面明确说明状态备份不是完整便携附件备份，不能据此建议清理 CaptureLog |
| REC-06 | Must | 补回 active / archived capture 的 payload 时允许越过普通“已导入则跳过”分支；deleted 墓碑仍不得复活 |
...
意图恢复率使用预先确定的 20 条样本，不挑容易记住的资料。每次样本捕获后，测试者在产品之外立即写一句真实保存原因；至少 24 小时后只查看 Inbox 内容，给出回忆并与隐藏基准比较，记为“清楚匹配 / 部分匹配 / 无法判断”。评估日志保留在本机，不提交仓库，也不发送给 AI。
...
- 恢复后 active / archived capture 显示 `needs-restore`；重新选择 CaptureLog 后可以补回原始 payload，deleted capture 不复活。
...
| 手动月份导入有摩擦 | 允许批量、明确当前月份；不伪装成自动同步 |
...
1. 这份文档是否已经清楚限定为 Capture Inbox 子系统，而不是完整自适应学习系统？
2. 阶段 A → B → C 是否足以先证明真实使用价值，仍有哪项长期工程错误地阻塞了 MVP？
3. `schemaVersion: 0` 真机 spike 后再冻结 v1 的顺序是否合理？
4. 50 次捕获、3 类来源、20 条延迟处理能否形成最低有效证据？
5. “零字段捕获”与意图丢失之间的验证方式是否足够？
6. 最小状态导出 / 空库恢复是否足以支持 Pilot，同时不演变成完整灾备项目？
7. 有序 `payloads[]`、最小 ImportLedger 和人工 promotion 的边界是否清楚？
8. GPT、完整 Vault 和原生 App 的后置条件是否合理？
9. 是否还有会导致用户再次弃用的日常操作摩擦？
